...
% INITIALISE VARS
load("Data/LR analyses/betas_signed_wo_rewunc_obj.mat","betas_all"); betas_subjs = betas_all; % participant betas
load("Data/LR analyses/betas_agent_recoding_wo_rewunc.mat","betas_agent"); % agent betas
load("Data/LR analyses/p_vals_signed_wo_rewunc_obj.mat","p_vals"); % p-vals from ttest
...
position_change = [0 0.03 0 0]; % change in position
...
if save_csv == 1
    ...
    writetable(save_table,strcat(save_figures,'\','figure4.csv'));
end
...
l = legend('','','Outcome','Dis-confirming outcome','Confirming outcome','Location','best','AutoUpdate','off');
l.EdgeColor = 'none';
l.Color = 'none';
l.ItemTokenSize = [7 7];
box off
...
annotation("textbox",[label_x label_y .05 .05],'String', ...
    'f','FontSize',fontsize_label,'LineStyle','none','HorizontalAlignment','center')
[label_x,label_y] = change_plotlabel(ax7_new,adjust_x,adjust_y);
annotation("textbox",[label_x label_y .05 .05],'String', ...
    'g','FontSize',fontsize_label,'LineStyle','none','HorizontalAlignment','center')
[label_x,label_y] = change_plotlabel(ax6_new,adjust_x,adjust_y);
%% SAVE FIGURE
