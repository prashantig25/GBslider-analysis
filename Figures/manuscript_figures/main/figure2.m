...
[~,~,~,~,color_screen,fb_green,darkblue_muted,mix,perc,rew,~,~,~,barface_green,...
    ~,~,~,~,~] = colors_rgb(); % colors
...
x = 1:25; % x-axis
hold on
lg_curves(x,mean_curves,sem_curves,colors_name,legend_names,title_name,xlabelname,ylabelname,font_size,1.5,font_name)
xlim([1,25])
...
bar_plots(y,mean_avg,mean_sd,num_subjs,length(mean_avg),length(legend_names), ...
    legend_names,xticks,xticklabs,title_name,xlabelname,ylabelname,font_size,linewidth_axes,font_name,colors_name) 
...
print(fig, fullfile(save_dir,filesep,"task1.png"), '-dpng', '-r1200') 