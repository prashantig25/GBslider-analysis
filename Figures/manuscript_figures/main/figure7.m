...
colors_name = [light_gray;mid_gray]; % colors for bars
...
jitter = rand(num_subjs,1);
for j = 1:length(jitter)
    jitter_rand = randi([1,2]);
    if jitter_rand == 1
        jitter(j) = jitter(j) * -0.1;
    else
        jitter(j) = jitter(j) * 0.1;
    end
end
...
for c = 1:num_conds
    ...
    differences = y_bar(:, 1) - y_bar(:, 2);
    positive_indices = differences > 0;
    negative_indices = differences <= 0;
    sorted_y_bar = [y_bar(positive_indices, :); y_bar(negative_indices, :)];
    hold on
    for i = 1:length(y_bar)
        plot(x_rep(i,:),sorted_y_bar(i,:), '-', 'color', dim_gray, 'linewidth', linewidth_single,'Marker','o','MarkerSize',2);
        hold on
    end
...
%     end
% end
% y_label = repelem(0.55,num_vars,1);
...
%     xticks,xticklabs,title_name,xlabelname,'Mean beta coefficients', ...
...
print(fig, 'figure7_2.png', '-dpng', '-r600') 