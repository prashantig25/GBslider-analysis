...
sigmas = 0.02;  % 10 sigma values from 0.001 to 0.05
alpha =  linspace(0.1, 0.3, 10) ;  % Fixed alpha for this test
kappa = 15;    % Fixed kappa for this test
n_trials_plot = 25;  % First 25 trials for visualization

% Collect mu_hat trajectories for each sigma
mu_trajectories = zeros(n_trials, length(alpha));

fprintf('Generating trajectories for %d different sigma values...\n', length(alpha));
for s = 1:length(alpha)
    rng(123);  % Fixed seed for fair comparison
    params_test = [alpha(s), kappa, sigmas];
    
    % Call your prediction function
    [~, mu_trajectories(:,s)] = predict_allModels.predict_basicRL(...
...
[xMask, yMask] = meshgrid(1:length(alpha));
...
tick_labels = arrayfun(@(x) sprintf('%.3f', x), alpha, 'UniformOutput', false);
xticks(1:length(alpha));
xticklabels(tick_labels);
xtickangle(45);
yticks(1:length(alpha));
...
subplot(1, 2, 2);
hold on;
colors = parula(length(alpha));

% Reshape trajectories into blocks (block_size x nBlocks x nSigmas)
mu_blocks = reshape(mu_trajectories, block_size, nBlocks, length(alpha));
...
for s = 1:length(alpha)
    ...
    plot(x, y, 'Color', colors(s,:), 'LineWidth', 1.5, ...
        'DisplayName', sprintf('\\sigma = %.3f', alpha(s)));
end
