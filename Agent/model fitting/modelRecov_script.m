...
data = importdata("genModel_Agent.mat");
...
parfor n = 1:numSubjs
    subj = data(data.ID == n, :);
    nll_fun = @(params) fitSlider_ALLmodels.nll_basicRL_integrated(params, subj.mu_hat, subj.blocks, subj.rewards, subj.condiff_relative);
...
safe_saveall('params_basicRL_RLSigmafixed.mat', params_basicRL);
safe_saveall('nll_basicRL_RLSigmafixed.mat', nll_basicRL);
...
parfor n = 1:numSubjs
    subj = data(data.ID == n, :);
    nll_fun = @(params) fitSlider_ALLmodels.nll_bayesianAgent(params, subj.mu_hat, subj, ...
        length(unique(subj.blocks)), 25, unique(subj.blocks), subj.rewards);
    options = optimset('Display', 'off');
    [params, nll] = fmincon(nll_fun, init_params, [], [], [], [], lb, ub, [], options);
...
safe_saveall('params_bayesianAgent_RLSigmafixed.mat', params_bayesianAgent);
safe_saveall('nll_bayesianAgent_RLSigmafixed.mat', nll_bayesianAgent);
...
parfor n = 1:numSubjs
    ...
    % Create logical mask where state equals choice
    mask = (subj.state == subj.choice);

    % Initialize confirm vector
    confirm = zeros(size(subj.rewards));

    % Assign values based on mask
    confirm(mask) = subj.rewards(mask);
    confirm(~mask) = 1 - subj.rewards(~mask);
    subj.confirm_rew = confirm;

    nll_fun = @(params) fitSlider_ALLmodels.nll_bayesianAgent_confirmBias(params, subj.mu_hat, subj, ...
        length(unique(subj.blocks)), 25, unique(subj.blocks), subj.rewards);
    options = optimset('Display', 'off');
    [params, nll] = fmincon(nll_fun, init_params, [], [], [], [], lb, ub, [], options);
...
safe_saveall('nll_bayesianAgent_confirmBias_Agentfixed.mat', nll_bayesianAgent_confirmBias);
safe_saveall('params_bayesianAgent_confirmBias_Agentfixed.mat', params_bayesianAgent_confirmBias);
...
parfor n = 1:numSubjs
    subj = data(data.ID == n, :);
    nll_fun = @(params) fitSlider_ALLmodels.nll_RLsigma_VOI(params, subj.mu_hat, subj.blocks, subj.rewards, subj.condiff_relative);
    options = optimset('Display', 'off');
    [params, nll] = fmincon(nll_fun, init_params, [], [], [], [], lb, ub, [], options);
    alphaParameter(n) = params(1);
    sigmaParameter(n) = params(3);
...
safe_saveall('params_RLsigma_RLSigmafixed.mat', params_RLsigma);
safe_saveall('nll_RLsigma_RLSigmafixed.mat', nll_RLsigma);
...
parfor n = 1:numSubjs
    ...
    confirm(~mask) = 1 - subj.rewards(~mask);
    subj.confirm_rew = confirm;
    nll_fun = @(params) fitSlider_ALLmodels.nll_RLsigma_confirmBias_VOI(params, subj.mu_hat, subj.blocks, subj.rewards, subj.condiff_relative, subj.confirm_rew);
    options = optimset('Display', 'off');
    [params, nll] = fmincon(nll_fun, init_params, [], [], [], [], lb, ub, [], options);
    sigmaParameter(n) = params(2);
    kappaParameter(n) = params(1);
...
safe_saveall('params_RLsigma_confirmBias_RLSigmafixed.mat', params_RLsigma_CB);
safe_saveall('nll_RLsigma_confirmBias_RLSigmafixed.mat', nll_RLsigma);
...
parfor n = 1:numSubjs
    ...
    confirm(~mask) = 1 - subj.rewards(~mask);
    subj.confirm_rew = confirm;
    nll_fun = @(params) fitSlider_ALLmodels.nll_basicRL_confirmBias_VOI(params, subj.mu_hat, subj.blocks, subj.rewards, subj.condiff_relative, subj.confirm_rew);
    options = optimset('Display', 'off');
    [params, nll] = fmincon(nll_fun, init_params, [], [], [], [], lb, ub, [], options);
    sigmaParameter(n) = params(2);
    kappaParameter(n) = params(1);
...
safe_saveall('params_basicRL_confirmBias_RLSigmafixed.mat', params_basicRL_CB);
safe_saveall('nll_basicRL_confirmBias_RLSigmafixed.mat', nll_RLsigma);

%% =================== COMPUTE AND SAVE AIC/BIC ==========================
nll_basicRL = importdata("nll_basicRL_Agent.mat");
nll_RLsigma = importdata("nll_RLsigma_Agent.mat");
nll_RLsigma_confirmBias = importdata("nll_RLsigma_confirmBias_modelAgent.mat");
nll_bayesianAgent = importdata("nll_bayesianAgent_Agent.mat");
nll_bayesianAgent_confirmBias = importdata("nll_bayesianAgent_confirmBias_Agentfixed.mat");
nll_basicRL_confirmBias = importdata("nll_basicRL_confirmBias_modelAgent.mat");

disp('Computing AIC and BIC for all models...');
% Number of trials per subject (assuming all subjects have same number)
num_trials = repelem(1000,numSubjs);%arrayfun(@(n) length(preprocess_fitSlider(data, uniqueID(n)).mu_hat,pupil), 1:numSubjs)';
% Model parameter counts (updated to include confirmation bias model)
num_params = [3, 3, 4, 2, 3, 4]; % [basicRL, RLsigma, PWRL, BayesianAgent, BayesianAgent_confirmBias]
...
% Save updated table
safe_saveall('AICBIC_Agentfixed.mat', AICBIC_table);
